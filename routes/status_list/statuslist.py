...
from utils.oidc4vc import alg, get_payload_from_token  # type: ignore
import json
...
ISSUER_KEY = json.dumps(json.load(open('keys.json', 'r'))['statuslist_private_key'])


def init_app(app):
    app.add_url_rule('/issuer/statuslist',  view_func=issuer_statuslist, methods=['GET', 'POST'])

    app.add_url_rule('/issuer/statuslist/<list_id>', view_func=issuer_status_list, methods=['GET'])

    app.add_url_rule('/connectors/issuer/statuslist/api', view_func=issuer_status_list_api, methods=['POST'])

    app.add_url_rule('/issuer/bitstringstatuslist',  view_func=issuer_bitstringstatuslist, methods=['GET', 'POST'])
    app.add_url_rule('/issuer/bitstringstatuslist/<list_id>', view_func=issuer_bitstring_status_list, methods=['GET'])

    app.add_url_rule('/issuer/statuslist/jwks', view_func=issuer_statuslist_jwks, methods=['GET'])

    app.add_url_rule('/issuer/statuslist/.well-known/openid-configuration', view_func=issuer_statuslist_openid, methods=['GET'])
 
    app.add_url_rule('/issuer/statuslist/.well-known/oauth-authorization-server', view_func=issuer_statuslist_openid, methods=['GET'])
...
@login_required
def issuer_statuslist():
    """
    UX to revoke sd-jwt with  ietf statuslist
    """
    mode = current_app.config["MODE"]
    if request.method == 'GET':
        return render_template("statuslist.html", user=current_user,)
    else:
...
@login_required
def issuer_bitstringstatuslist():
    """
    UX to revoke vc_jwt_json with w3c bitstring statuslist
    """
    mode = current_app.config["MODE"]
    if request.method == 'GET':
        return render_template("bitstringstatuslist.html", user=current_user)
    else:
        index = request.form['index']
        if request.form["button"] == "active":
...
def sign_status_list_bitstring_credential(lst, list_id, mode):  # for sd-jwt
    ...
    payload = {
        "iat": round(datetime.timestamp(datetime.now())),
        "jti": mode.server + "issuer/bitstringstatuslist/" + list_id,
        "vc": {
            "@context": [
                "https://www.w3.org/ns/credentials/v2",
                "https://www.w3.org/ns/credentials/status/v1"
            ],
            "id": mode.server + "issuer/bitstringstatuslist/" + list_id,
            "type": ["VerifiableCredential", "BitstringStatusListCredential"],
            "issuer": mode.server + "issuer/statuslist",
            "validFrom":  datetime.now().replace(microsecond=0).isoformat() + "Z",
            "credentialSubject": {
                "id": mode.server + "issuer/bitstringstatuslist/" + list_id + "#list",
        ...
        "sub": mode.server + "issuer/bitstringstatuslist/" + list_id,
        #"exp": round(datetime.timestamp(datetime.now())) + 365*24*60*60,
        "iss": mode.server + "issuer/statuslist",
    }
    token = jwt.JWT(header=header, claims=payload, algs=[alg(key)])
