...
def oidc_authorize():
    ...
    logging.info("authorization request: %s", dict(request.args))
...
def build_jwt_request(account, credential_id, jwt_request, client_id_scheme) -> str:
    ...
    if header["alg"] == "none":
        ...
        return signer.sign_jwt(account, credential_id, private_key, credential.credential_type, header, payload)
...
def signin_qrcode():
    ...
    authorization_request = { 
        ...
        "response_type": code_data["response_type"],
    ...
    # SIOPV2
    if 'id_token' in code_data["response_type"]:
    ...
    request_as_jwt = build_jwt_request(account, credential_id, authorization_request, signin.client_id_scheme)
