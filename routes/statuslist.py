...
def init_app(app, red, mode):
    ...
    app.add_url_rule('/issuer/statuslist/api', view_func=issuer_status_list_api, methods=['POST'],  defaults={"mode": mode})

    app.add_url_rule('/issuer/bitstringstatuslist',  view_func=issuer_bitstringstatuslist, methods=['GET', 'POST'], defaults={"mode": mode})
...
def issuer_bitstringstatuslist(mode):
    ...
    if request.method == 'GET':
        ...
        if request.form["button"] == "active":
            ...
            logging.info("revoke index = %s", index)
        return redirect("/issuer/bitstringstatuslist")
...
def issuer_status_list_api(mode):
    ...
    curl -d "status=false" -d "index=1000" -H "Content-Type: application/x-www-form-urlencoded"  -H "X-Api-Key: 123456" -X POST http://192.168.0.20:3000/issuer/statuslist/api
    curl -d "status=suspended" -d "index=5320" -H "Content-Type: application/x-www-form-urlencoded"  -H "X-Api-Key: 123456" -X POST https://talao.co/issuer/statuslist/api
