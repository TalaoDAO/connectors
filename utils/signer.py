...
# ---- main function ----
def sign_jwt(account, credential_row, header, payload):
    ...
    elif credential_row.credential_type == "qualified":
        signer = TrustedAppSigner(account)
        signer.select_credential(credential_row.credential_id)
        signing_input = build_signing_input(header, payload)
        signature = signer.sign_hash(signing_input.encode())
        sig_b64url = normalize_b64_to_b64url(signature)
